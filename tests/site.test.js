...
test('esquema elétrico contém os componentes de segurança e o gerador', () => {
  ...
  for (const s of ['GERADOR', 'CHAVE REVERSORA', 'T 200 A', 'Disj. DC 2P', 'DPS DC', 'Shunt', 'BARRAMENTO PE', 'QDC AC', 'IDR 30 mA', 'INVERSOR']) {
...
test('seções esperadas presentes', () => {
  for (const id of ['alerta', 'resumo', 'calc', 'baterias', 'esquema', 'gerador', 'config', 'riscos', 'checklist', 'fontes']) {
...
test('conteúdo: risco crítico 24/48 V e Isc; programas essenciais do inversor', () => {
  assert.ok(T.risks.some((r) => r.sev === 'high' && /48 V/.test(r.risk)));
